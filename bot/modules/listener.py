...
class MirrorLeechListener:
    ...
    def onUploadComplete(self, link: str, size, files, folders, typ, name):
        ...
        if self.isLeech:
            ...
            if not files:
                if PICS:
                    uploadmsg = sendPhoto(msg, self.bot, self.message, random.choice(PICS), userbutton.build_menu(2))
                else:
                ...
                for index, (link, name) in enumerate(files.items(), start=1):
                    ...
                    if len(fmsg.encode() + msg.encode()) > 2000:
                        ...
                        if FORCE_BOT_PM is False:
                            if PICS:
                                uploadmsg = sendPhoto(msg + fmsg + pmwarn + logleechwarn + warnmsg, self.bot, self.message, random.choice(PICS), userbutton.build_menu(2))
                            else:
                                uploadmsg = sendMarkup(msg + fmsg + pmwarn + logleechwarn + warnmsg, self.bot, self.message, userbutton.build_menu(2))
                            Thread(target=auto_delete_upload_message, args=(bot, self.message, uploadmsg)).start()
                        fmsg = ''
                if fmsg != '':
                    sleep(1.5)
                    ...
                    for i in LEECH_LOG:
                        ...
                        for index, (link, name) in enumerate(files.items(), start=1):
                            ...
                            if len(indexmsg.encode() + msg.encode()) > 4000:
                                result = bot.sendMessage(chat_id=i, text=msg + indexmsg,
                                                reply_markup=buttons.build_menu(2),
                                                parse_mode=ParseMode.HTML)
                                indexmsg = ''
                                result_link = result.link
                                userbutton.buildbutton("⭐Log🌈", result_link)
                        if indexmsg != '':
                                result = bot.sendMessage(chat_id=i, text=msg + indexmsg,
                                                reply_markup=buttons.build_menu(2),
                                                parse_mode=ParseMode.HTML)
            ...
            userbutton.buildbutton("⭐Log🌈", logmirresult_link)        
            if BOT_PM and self.message.chat.type != 'private':	
                try:	
